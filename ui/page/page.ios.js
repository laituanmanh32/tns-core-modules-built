...
function isBackNavigationTo(page, entry) {
    ...
    if (!frame || frame.navigationType === frame_common_1.NavigationType.replace) {
        return false;
    }
...
var UIViewControllerImpl = (function (_super) {
    ...
    UIViewControllerImpl.prototype.viewDidAppear = function (animated) {
        ...
        if (!owner._presentedViewController && frame) {
            var newEntry = this[ENTRY];
            var isBack = void 0;
            var navType = frame.navigationType;
            if (navType !== frame_common_1.NavigationType.replace && frame.currentPage === owner && frame._navigationQueue.length === 0) {
                isBack = false;
                navType = frame_common_1.NavigationType.forward;
            }
            else {
                isBack = isBackNavigationTo(owner, newEntry);
                if (isBack) {
                    navType = frame_common_1.NavigationType.back;
                }
            }
            frame.setCurrent(newEntry, navType);
            if (frame.navigationType === frame_common_1.NavigationType.replace) {
            ...
            frame.navigationType = isBack ? frame_common_1.NavigationType.back : frame_common_1.NavigationType.forward;
